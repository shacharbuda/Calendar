...
namespace Calendar
{
    public partial class FormRptEventsByMember : Form
    {
        ...
        public FormRptEventsByMember(OleDbConnection dataConnection,bool isAdmin)
        {
            InitializeComponent();
            this.dataConnection = dataConnection;
            RefreshDataGridView();
            if (!isAdmin)
                dataGridView2.Columns.Remove("userPasswordDataGridViewTextBoxColumn");
        }

        private void FormRpyEventsByMemberID_Load(object sender, EventArgs e)
        {
        ...
        private void FillSelectedRow()
        {
            try
            {
                eventMemberID = dataGridView2.SelectedRows[0].Cells[0].Value.ToString();
        ...
        private void GetEvents()
        {
            try
            {
                ...
                OleDbCommand datacommand = new OleDbCommand();
                datacommand.Connection = dataConnection;
                datacommand.CommandText = "SELECT   eventID, eventType, eventName, eventDate, eventTime, eventMinutes, eventPicture " +
                                         "FROM      tblEvents   " +
        ...
        private void listView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            while (memberID == "")
            {
                memberID = listView1.Items[selectedIndex - j].SubItems[0].Text;
                j++;
            }
            int i = 1;
