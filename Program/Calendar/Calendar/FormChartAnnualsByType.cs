...
namespace Calendar
{
    public partial class FormChartAnnualsByType : Form
    {
        ...
        public FormChartAnnualsByType(OleDbConnection dataConnection)
        {
            InitializeComponent();
        ...
        private void FillEvents()
        {
            try
            {
                ...
                for (int i = 0; i < types.Length; i++)
                {
                    OleDbCommand datacommand = new OleDbCommand();
                    datacommand.Connection = dataConnection;
                    datacommand.CommandText = "SELECT  annualYear, annualMonth, annualDay " +
                                              "FROM    tblAnnuals  " +
                                              "WHERE   annualType = \"" + types[i] + "\"";
                    OleDbDataReader dataReader = datacommand.ExecuteReader();
                    while (dataReader.Read())
                    {
